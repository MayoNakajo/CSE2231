...
public final class BugsWorldVMInterpreter {
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs, int pc) {
        ...
        // pc - program counter
        int num = 0;
        int jump = Instruction.JUMP.byteCode();

        if (!isPrimitiveInstructionByteCode(cp[pc])) {
            // if instruction matches cell state of EMPTY, WALL, FRIEND, ENEMY
            if (cp[pc] == Instruction.JUMP.byteCode()) {
                // else, if cp[pc] is JUMP pc = cp[pc+1] return recursive call
                System.out.println("cp[pc + 1] : " + cp[pc + 1]);
                num = nextPrimitiveInstructionAddress(cp, wbs, cp[pc + 1]);
            } else if (conditionalJumpCondition(wbs, cp[pc])) {
                // else we have conditional jump
                // if we take the jump, pc = cp[pc+1] return recursive call
                System.out.println("cp[pc + 1] : " + cp[pc + 1]);
                System.out.println("pc : " + pc);
                num = nextPrimitiveInstructionAddress(cp, wbs, cp[pc + 1]);
            } else {
                // else pc = pc+2 return recursive call
                System.out.println("pc + 2 : " + pc + 2);
                num = nextPrimitiveInstructionAddress(cp, wbs, pc + 2);
            }
        } else {
            // if cp[pc] is primitive, return pc
            num = pc;
        }
...
//        // set num to pc
//        num = pc;
//        // loop until cp[num] is primitive
//        while (!isPrimitiveInstructionByteCode(cp[num])) {
//            if (cp[num] == Instruction.JUMP.byteCode()) {
//                // if cp[num] is JUMP, num = cp[num + 1]
//                num = cp[num + 1];
//            } else if (!conditionalJumpCondition(wbs, cp[pc])) {
//                // else we have conditional JUMP
//                // if we take the JUMP, cum = cp[num + 1]
//                num = cp[num + 1];
//            } else {
//                // else num = pc + 2
//                num = num + 2;
...
//        }
    ...
    public static void main(String[] args) {
        ...
        out.println();
        out.println(
                "Enter program counter outside the [0," + cp.length + ") range to quit.");
        /*
        ...
        while (true) {
            ...
            if (input.length() > 0 && !FormatChecker.canParseInt(input)) {
                out.println("Program counter must be a number in the [0," + cp.length
                        + ") range");
                continue;
            ...
            out.println("  Next primitive instruction: "
                    + Program.Instruction.values()[cp[pc]].toString() + " at address "
                    + pc);
            /*
             * Output disassembled program with marked address
             */
